...
if (isAdmin()) {
    setFlash('info', __('promo.admin_redirect'));
    redirect(BASE_URL . 'admin/promotion_payments.php');
}

$currentUserId = currentUserId();
$pageTitle = __('promo.page_title');
...
if (!$promoPaymentsTableExists) {
    setFlash('error', __('promo.feature_inactive'));
    ...
    <div class="container" style="max-width: 1000px;">
        <?php if (isset($_GET['new_listing']) && $_GET['new_listing'] == '1'): ?>
            <div class="mb-8 p-4 text-center" style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: var(--radius-lg);">
                <h2 style="color: #15803d; font-size: 1.25rem; font-weight: 800; margin-bottom: 0.5rem;"><?= __('promo.new_listing_title') ?></h2>
                <p style="color: #166534; font-size: 0.95rem; margin-bottom: 0.5rem;"><?= __('promo.new_listing_body') ?></p>
                <a href="<?php echo BASE_URL; ?>pages/profile.php" style="color: #166534; text-decoration: underline; font-size: 0.85rem; font-weight: 600;"><?= __('promo.new_listing_skip') ?></a>
            </div>
        <?php endif; ?>
        ...
        <div class="flex flex-col lg:flex-row gap-10 items-stretch">
            ...
            <div class="glass-panel" style="flex: 1.3; width: 100%; border-radius: 32px; border: 1px solid rgba(99, 102, 241, 0.15); box-shadow: 0 25px 60px rgba(0,0,0,0.08); background: white; padding: 2rem; display: flex; flex-direction: column;">
                <h3 class="mb-6 font-bold text-main uppercase tracking-widest text-center" style="font-size: 1rem; letter-spacing: 0.15em;"><?= __('promo.configure') ?></h3>
                
                <div class="mb-8 p-4" style="background: rgba(99, 102, 241, 0.03); border-radius: 20px; border: 1px dashed rgba(99, 102, 241, 0.2);">
                    <label class="block mb-2 font-bold" style="font-size: 0.85rem; color: var(--primary); text-transform: uppercase;"><?= __('promo.select_listing') ?></label>
                    <select name="product_id" id="product_id" class="form-control" style="border-radius: 12px; border: 1px solid #e2e8f0; font-weight: 500;">
                        <option value=""><?= __('promo.select_placeholder') ?></option>
                        <?php foreach ($myProducts as $p): ?>
                            <option value="<?php echo (int)$p['id']; ?>" <?php echo ($prefillProductId > 0 && (int)$p['id'] === $prefillProductId) ? 'selected' : ''; ?>>
                                <?php echo sanitize($p['title']); ?> (<?php echo (int)$p['is_featured'] === 1 ? __('promo.already_featured') : __('promo.not_featured'); ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="mb-8">
                    <label class="block mb-4 font-bold text-center" style="font-size: 0.85rem; color: var(--text-muted); text-transform: uppercase;"><?= __('promo.choose_amount') ?></label>
                    ...
                    <input type="number" id="custom-amount" class="premium-input mt-4 text-center" placeholder="<?= addslashes(__('promo.custom_placeholder')) ?>" style="border-radius: 16px; font-size: 1.1rem; font-weight: 700;">
                    <p class="text-center mt-2 text-muted" id="duration-preview" style="font-size: 0.8rem;"><?= __('promo.duration_enter') ?></p>
                </div>
                ...
                <form action="create_stripe_session.php" method="POST" id="stripe-form">
                    ...
                    <div class="flex items-center justify-center gap-4 mt-8" style="padding-top: 1rem; border-top: 1px solid #f1f5f9;">
                        <div class="flex items-center gap-2 opacity-60">
                            ...
                            <span class="font-bold uppercase tracking-widest" style="font-size: 0.65rem;"><?= __('promo.auto_activation') ?></span>
                        </div>
            ...
            <div style="flex: 0.7; display: flex; flex-direction: column; gap: 1.5rem;">
                <div class="glass-panel" style="padding: 1.75rem; border-radius: 28px; background: rgba(255,255,255,0.7); border: 1px solid rgba(0,0,0,0.03);">
                    <h4 class="font-bold mb-4" style="font-size: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                        <svg style="width: 20px; height: 20px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> <?= __('promo.status_title') ?>
                    </h4>
                    
                    <?php if (empty($payments)): ?>
                        <div class="text-center py-8 opacity-50">
                            <p style="font-size: 0.9rem;"><?= __('promo.no_boosts') ?></p>
                        </div>
                    <?php else: ?>
                            ...
                            <?php foreach (array_slice($payments, 0, 5) as $pay): ?>
                                <div style="background: white; padding: 1rem; border-radius: 16px; box-shadow: var(--shadow-sm); border: 1px solid #f1f5f9;">
                                    <div class="flex justify-between items-start mb-1">
                                        <span class="font-bold" style="font-size: 0.85rem; color: var(--text-main);"><?php echo sanitize($pay['product_title'] ?: __('promo.listing_boost')); ?></span>
                ...
                <div class="glass-panel" style="padding: 1.75rem; border-radius: 28px; background: var(--primary); color: white; box-shadow: 0 10px 20px rgba(79, 70, 229, 0.2);">
                    <h4 class="font-bold mb-2" style="color: white; font-size: 1.1rem;"><?= __('promo.seller_tip_title') ?></h4>
                    <p style="font-size: 0.95rem; color: #f8fafc; font-weight: 500; opacity: 1; line-height: 1.6; margin-bottom: 0;"><?= __('promo.seller_tip_body') ?></p>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const promoI18n = {
        previewTpl: <?= json_encode(__('promo.duration_preview')) ?>,
        oneDay: <?= json_encode(__('promo.duration_one_day')) ?>,
        manyDays: <?= json_encode(__('promo.duration_many_days')) ?>,
        enterAmount: <?= json_encode(__('promo.duration_enter')) ?>,
        alertListing: <?= json_encode(__('promo.alert_select_listing')) ?>,
        alertAmount: <?= json_encode(__('promo.alert_valid_amount')) ?>
    };
    ...
    function updatePreview(val) {
        ...
        if (days > 0) {
            const text = days === 1
                ? promoI18n.oneDay
                : promoI18n.manyDays.replace('{n}', String(days));
            previewEl.innerHTML = promoI18n.previewTpl.replace('{text}', text);
        } else {
            previewEl.innerHTML = promoI18n.enterAmount;
        }
    }
